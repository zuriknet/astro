...
import type { AstroConfig, RendererConfig } from '../@types/astro';
...
const JSX_RENDERER_CACHE = new WeakMap<AstroConfig, Map<string, RendererConfig>>();
...
function collectJSXRenderers(renderers: RendererConfig[]): Map<string, RendererConfig> {
	const renderersWithJSXSupport = renderers.filter((r) => r.jsxImportSource);
	return new Map(renderersWithJSXSupport.map((r) => [r.jsxImportSource, r] as [string, RendererConfig]));
}
	...
	return {
		...
		async transform(code, id, opts) {
			...
			if (!jsxRenderers) {
				...
				const possibleRenderers = await collectJSXRenderers(config._renderers);
			...
			if (importSource) {
				...
				return await transformJSX({ code: jsxCode, id, renderer: jsxRenderers.get(importSource) as RendererConfig, mode, ssr });
			}
